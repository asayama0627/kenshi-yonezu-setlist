  ...
  return <main>
    ...
    <section className="database-section">
      ...
      {view==="live"?<>
        ...
        <div className="release-list">{groups.map(g=><details className="release-card release-accordion" key={g.name}><summary><div><span>{g.type}</span><h3>{g.name}</h3><small>{g.songs.length}曲</small></div><b>{g.year}</b></summary><div className="song-table"><div className="song-head"><span>楽曲</span><span>確認済み演奏回数</span></div>{g.songs.map((s,i)=><div className="song-row" key={s.name}><span><i>{String(i+1).padStart(2,"0")}</i>{s.name}</span><button className="performance-link" onClick={()=>setSongOpen(s.name)} aria-label={`${s.name}が演奏されたライブを見る`}><strong>{s.confirmed}<small>回</small></strong><em>公演を見る →</em></button></div>)}</div></details>)}</div>
      </>}
    </section>
